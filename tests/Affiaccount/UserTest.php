...
class UserTest extends TestCase
{
    ...
    public function testBuild(): void
    {
        ...
        static::assertSame('https://api.weixin.qq.com/cgi-bin/user/get', $requestOptions->getUrl());
        static::assertEquals([
            'query' => [
                'access_token' => 'foo',
            ],
    ...
    {
        $data = ['data' => ['openid' => ['foo', 'bar', 'baz']]];
        $body = json_encode($data, \JSON_THROW_ON_ERROR);

        $mockResponse = new MockResponse($body);
        $client = new MockHttpClient($mockResponse);

        $result = (new User($client))->send(['token' => 'foo']);
        static::assertSame($data['data']['openid'], $result);
    }
    ...
    public function testSendWithParseResponseException(): void
    {
        ...
        (new User($client))->send(['token' => 'foo']);
    }
